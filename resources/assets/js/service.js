...
if ($('body[view-name="serviceform"]').length > 0) {
    window.vue = new Vue({
        el: '#app',
        components: {
            Modal,BasicSelect
            ...
            onSuccess(response) {
                try {
                    if (response.data.result == "true") {
                        ...
                        this.$refs.modal.configModal('Sucesso', 'Serviço cadastrado!', 'OK', '', function () {
                            util.goBack();
                        });
...
if ($('body[view-name="serviceindex"]').length > 0) {
    window.vue = new Vue({
        el: '#app',
        components: {
            Modal,BasicSelect
        ...
        methods: {
            activeDisabled(id,type)
            {
                ...
                this.$refs.modal.configModal('Aviso', msn, btn, 'Cancelar', function () {
                    let url = window.baseUrl+'/service/activeDisabled';
                    originalData.form.post(url, {id: id, type: type}, originalData.onSuccessActiveDisabled);
                });
                this.$refs.modal.show();
            },
